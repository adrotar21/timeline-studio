...
|---|-------|-------------|------|----------|--------|
| B46 | **TTT offset color: green for positive/zero values** | Time-to-target offset labels currently use a single color for all values. Positive offsets (ahead of target) and zero (exactly on target) should display in green to indicate "on track" status, while negative offsets (behind target) should display in red/accent. Applies to on-screen timeline labels and export SVG. Small CSS + render logic change. | XS | :orange_circle: P1 | :white_circle: Open |
| B45 | **Data table status dropdown flash on unselected row** | When clicking a status dropdown in Data View on a row that isn't already the sole selection, the dropdown briefly flashes open then disappears as the row gets selected — user must click again to actually edit. B39 fixed the case where the row was already selected; this is the remaining case where selection change triggers a re-render that destroys the dropdown before the user can interact with it. Fix: suppress the dropdown open (or defer it) until after selection settles, so the user never sees the flash. The re-click-to-edit behavior is correct — just hide the initial flash. | XS | :orange_circle: P1 | :white_circle: Open |
...
| F53 | **File handle UX on website load** | Investigate and improve the user experience when opening the hosted website and content is loaded from localStorage (no file handle). Currently there's no indication that the user doesn't have a file handle — they may edit for a while and not realize they can't Ctrl+S to a file. **Proposed:** (1) Detect when a project loads from localStorage without a `_fileHandle` and show a subtle, non-blocking indicator (e.g. status bar hint or toolbar badge) that says "Working from memory — Save to create a file" or similar. (2) Consider auto-setting and leveraging a project folder: on first Save, remember the directory via File System Access API's `showDirectoryPicker()` or the save location from `showSaveFilePicker()`, and use that as the default for future saves and HD screenshot auto-save (F46). (3) On subsequent visits, if localStorage has data but no handle (handles don't persist across sessions), prompt or hint to re-open the file to re-establish the handle. **Questions:** Should the app auto-prompt "Open existing file?" on load when localStorage has data? Should there be a persistent "📁 Not saved to file" indicator vs "📁 ProjectName.tlproj" indicator? | M | :orange_circle: P1 | :white_circle: Open |
| F45 | **View scale change smart zoom** | When switching view scales (e.g. months → days), the timeline should auto-fit intelligently rather than cramming everything into the viewport. Current behavior: switching to days view auto-fits and becomes very cramped. **Smart-aware zoom:** Instead of strict fit-to-content, center the view around the today marker (or selection centroid if items are selected) and choose a zoom level that shows a comfortable number of day columns on the header — enough to read without being cramped. Could use a heuristic: target ~20-40 visible columns at the new scale, centered on today. For scales with very many columns (days), prefer showing a readable window rather than the full range. For scales with few columns (years), standard fit-to-content is fine. The behavior should feel like "zoom to comfortable reading level at the new scale" rather than "cram everything in." | M | :orange_circle: P1 | :white_circle: Open |
...
| F25 | **Item links/URLs** | **[Plan — needs refinement/discussion]** Allow tasks and milestones to store one or more hyperlinks, with a designated **primary link**. **Data model:** Each item gets a `links` array of objects, each with: URL, Display Name (optional — falls back to URL), Link Type/category (e.g. "JIRA", "Confluence", "SharePoint", "Other"), and `isPrimary` boolean (exactly one per item). The primary link is the default click-through and the one shown prominently on the timeline. **Paste-to-create links:** Copying and pasting a URL or list of URLs (return, comma, or tab separated) onto a selected item auto-generates link entries. If the item has no primary link, the first pasted URL becomes primary. **Properties pane UI:** A links section in the item properties pane — add/remove/edit links, star/unstar primary, each rendered as a clickable hyperlink that opens in a new tab. Compact display (icon + short name) with expand/edit on click. **Configuration:** Project-level settings for default link types/categories (so users can predefine "JIRA", "Wiki", etc. with URL templates like `https://jira.company.com/browse/{key}`). **Timeline display:** Optional — small link icon badge on items that have links (similar to pin badge). Click or hover to reveal link list; primary link accessible via single click or keyboard shortcut. **Data table:** Links column showing primary link (or count if multiple), with expand to see all. **Export:** Links are metadata-only in PNG export (no clickable links in images). SVG export could include `<a>` elements for clickable links. CSV export includes links as a delimited string (primary first). **Open questions:** Maximum number of links per item? Integration with paste-import from Excel (link column)? Should the timeline badge distinguish primary vs secondary links visually? | M | :large_blue_circle: P3 | :memo: Plan |
...
- **B46** — TTT offset color: green for positive/zero (XS)
- **B45** — Data table status dropdown flash on unselected row (XS)
- **F53** — File handle UX on website load (M)
- **F45** — View scale change smart zoom (M)
