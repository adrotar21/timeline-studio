...
|---------|------|---------|
| **0.42.3** | 2026-02-25 | B49 share link fix for Office apps: embedded hyperlinks from Word/Excel URL-encode `#` as `%23`, so `location.hash` is empty and `_loadFromHash()` silently fell through to `loadAuto()`. Added `%23p=` fallback in `_loadFromHash()` — checks full `location.href` when hash is absent. Added `console.warn` to catch block for future debugging. |
