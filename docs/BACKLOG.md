...
|---------|------|---------|
| **0.44.11** | 2026-03-04 | B55 fix: Dropdown emoji icons greyed out on Mac Safari. The broad CSS rule `.save-dd-item span{color:var(--acc)}` applied the theme accent color to all `<span>` elements inside dropdown items, including emoji icon spans. Windows Chrome ignores CSS `color` on emoji glyphs, but Mac Safari forces them through the text rendering path, producing monochrome/tinted icons. Fix: removed `color:var(--acc)` from the broad span rule and targeted it to `.save-dd-item .dd-shortcut` only. Emoji icons now inherit parent text color and render in native color on all platforms. CSS-only change. |
