...
|---------|------|---------|
| **0.48.0** | 2026-06-12 | **F52 Compact timeline view — Row Density.** New "Row Density" selector (Normal / Compact) in the View dropdown directly under Scale, for vertical space efficiency on long execution-tracking timelines. (1) **`DENSITY_PRESETS` table** (app.js, before the App object) is the single source of truth for vertical geometry — `normal:{rH:38,bar:22,subMin:50,slDef:120,hKey:'height'}`, `compact:{rH:26,bar:15,subMin:36,slDef:40,hKey:'heightC'}`. Extensible by design: adding a future level (e.g. `tight`) = one table entry + one `<option>` in `#density-sel`; all row math, drag geometry, export SVG, and the toggle shortcut read from the table. (2) **`_rGeom()` helper** resolves the active preset from `proj.rowDensity` (defaulted in `newProj()`, validated in `migrate()`, stripped from share links via `projStrip` when default — same lifecycle as `dayColumnWidth`). (3) **Per-density lane heights**: each preset declares which lane field stores manual resizes (`hKey` — Normal→`sl.height`/`ss.height`, Compact→`heightC`). Entering Compact shrink-wraps (compact fields start unset → every lane packs to content, `(maxRow+1)×26+10`, floors 36 sub / 40 lane), but lanes **stay fully resizable** — drags write only the active density's field, so the two layouts never contaminate each other and switching densities always restores that mode's exact layout. Compact resizes persist across switches/saves/reloads; `_packProj` strips unset `heightC`. `bindRH()` resolves density at drag start and seeds drags from the *rendered* height (label element) rather than the stored field, eliminating the dead-zone when dragging a content-packed lane. `autoFitHeights()` is density-aware (writes the active `hKey` with that density's geometry) — in Compact it doubles as "re-shrink-wrap". **Minimized-neighbor edge fix:** the sub divider is a resize handle whenever the sub *above* it (the one it resizes) is expanded — previously it also required the sub *below* to be expanded, stranding an expanded sub above a minimized one; and the lane-bottom handle now targets the last *expanded* sub instead of the array-last sub (which, when minimized, rendered at a fixed 20px so drags wrote to a visually-ignored field and snapped back; all-subs-minimized → no-op). Invariant: every expanded sub is always resizable regardless of neighbors' collapse state. (4) **All former `rH=38`/`barH=22` literals parameterized**: `renderTL()`, `showCtx()` right-click row math, drag-move CSS-expansion + ghost previews, drop handler, and `buildExportSVG()` — exports/screenshots are WYSIWYG per density. TTT + float label offsets now bar-derived (screen `iy+bar+1`, export `iy+bar+10`). Task bar height flows through a new `--bar-h` CSS var (set in `renderTL()` alongside `--sl-w`; `.tl-task-bar{height:var(--bar-h,22px)}`) so progress fill, % text, resize grips, and label centering adapt automatically. Milestone icons stay 16px at all densities. (5) **UI/UX**: select carries an explanatory tooltip; re-synced in the rAF render loop so undo/load/share-link arrivals stay consistent. New `toggleDensity` shortcut action (cat View, no default binding) cycles preset keys. (6) **Auto-arrange needed zero changes** — `_autoLayoutItems()` row budget is item-density-based, not pixel-height-based. (7) **85 new tests** in `tests/features/test_row_density.js`: preset↔engine consistency, migration, per-density lane-height formulas, density isolation (compact resizes/auto-fit never leak into normal fields), click/drag row-mapping round-trips, ≥35% vertical-savings scenario, density-aware autoFit + bindRH wiring, full UI/CSS wiring. 31/31 test files passing. Browser-verified on a seeded 4-lane project: 786px → 340px body height (−57%); simulated handle drags in Compact (+50 lane / +30 sub) resize live with no dead-zone; switching to Normal restores [162,220,284,120] exactly; back to Compact restores the compact resizes; compact export SVG bars at 15px. |
| **0.47.0** | 2026-04-23 | **F61 My Views — saved swimlane layouts + Return to Previous.** New per-timeline feature lets users save named configurations of swimlane/sub-swimlane collapse state and swap between them instantly. Addresses beta feedback from Sean (PgM) who works in a "grey area" of mixed collapse — Collapse All + Expand All overshot the layout he actually wanted to return to. (1) **Schema v2→v3**: new `proj.views[]` array with sparse entries `{id, name, created, modified?, slStates:{<slId>:{c?, subs?}}}`. Sparse encoding drops `'expanded'` defaults (matches `_packProj()` convention); empty `views:[]` stripped from share-links. Migration idempotent, normalizes missing fields. (2) **Core methods** at app.js:~2963: `_captureSlSnapshot({dense})` (sparse for saved views, dense for `_prevView` so full restore works even when lanes drifted between capture and return), `_applySnapshot()` with soft ID-match (known IDs applied, unknown current IDs untouched, missing saved IDs silent-skip), `captureCurrentView/overwriteView/updateView/updateLastAppliedView/applyView/returnToPrevView/deleteView/renameView/clearAllViews`. Drift info surfaced in apply toast: "Applied 'X' — 2 new swimlanes unchanged, 1 missing". Max 20 views per project. (3) **UI integration** in View dropdown Swimlanes section (index.html:131-138): new "↩ Return to previous view" row (auto-disabled until first Expand/Collapse All or Apply View in session) and "My Views ▸" nested panel with count badge. Panel rows show `●` last-applied indicator, inline ⟳ update-with-current / ✎ rename / ✕ delete (all with inline confirmation, matching holiday-modal pattern). Save popover with duplicate-name detection → Cancel/Rename/Overwrite flow. (4) **Manage Views modal**: full list with Apply/Update/Rename/Delete per row, "Updated <date>" vs "Created <date>" label based on `modified` field, save-as-new + clear-all footer actions. (5) **4 new keyboard shortcuts** registered in `SHORTCUT_ACTIONS` (cat:View, defaults:[]): `returnToPrevView`, `saveCurrentView`, `updateLastAppliedView`, `openManageViews`. All customizable via Settings → Shortcuts; auto-surfaced in help modal. (6) **`expandAll`/`collapseAll`** instrumented to capture dense `_prevView` snapshot before mutation at both dispatch (app.js:489-490) and inline click-handler paths. Complements existing `snap()` undo — `returnToPrevView` is additive (toggles A↔B, doesn't consume undo slot, only touches swimlane state). Session-only transient state (`_prevView`, `_lastAppliedViewId`) not persisted — matches `_lastShiftSel` pattern. (7) **30 new tests** in `tests/features/test_views.js` (58 assertions): sparse/dense capture, apply with drift counts, oscillation invariant, overwrite preserves `created`/sets `modified`, delete clears `_lastAppliedViewId`, v2→v3 migration idempotence, `_packProj` roundtrip. 31/31 test files passing. |
| **0.46.6** | 2026-04-16 | Tier A audit bundle: 5 correctness & UX fixes from targeted bug-hunt sweep. (1) **H1 Zero-duration task endDate freshening** — `_recalcNonWorkingDays()` (app.js:1847), `calculateFloat()` (app.js:1897), and `runSchedule()` (app.js:2046) each guarded endDate refresh with `&&it.duration`, which is falsy for `duration=0`. Zero-duration tasks therefore skipped refresh and could carry stale `endDate` values into float calculation and scheduling, producing spurious float=1 results and wrong dependency anchors. Changed all three to `&&it.duration!=null`, matching the correct idiom already used in `migrate()` at app.js:682. (2) **M1 Resize duration clamp** — `startTR()` (app.js:5953) recomputed duration from date-diff after clamping crossed dates to equal, which in work-mode could yield `duration=0` if both start and end landed on a weekend/holiday. Wrapped in `Math.max(1, ...)`. (3) **M4 Export DPR downscale toast** — `copyScreenshot()` (app.js:5977) and `exportPNG()` (app.js:6765) silently reduced DPR (3× → 1×) when canvas would exceed the 16384-px cap on huge day-scale timelines. Now captures `origDpr` and toasts `"Large timeline — reduced export resolution (N× → M×)"` when the reduction kicks in, so the user knows why the export looks softer. (4) **U4 Multi-delete confirm** — `deleteSel()` (app.js:3173) deleted selections silently regardless of size or pin state. Now prompts `"Delete N items?"` for any selection of 2+ OR any single pinned item; single non-pinned deletions remain silent to avoid nag. (5) **U6 Dev Panel warning banner** — `#dev-panel` modal (index.html:665) now shows a red-tinted warning strip above the Kill Switch section explaining that changes override the real license system and that closing without clicking Apply reverts. Prevents accidental `Ctrl+Shift+L` triggers from silently breaking feature gates for confused users. Findings came from a 12-pass architectural audit spanning engine correctness, popover/drag state, scroll sync, overlay padding, IDB transactions, license recovery, import, export, and usability. All 464/464 assertions across 29 test files pass. |
| **0.46.5** | 2026-03-28 | B58 fix: Import milestone date fallback ignoring end date. When importing rows with no start date but an end date, `doAdvancedImport()` milestone path used `startD || new Date()`, skipping `endD` entirely — milestones got today's date instead of the mapped end/due date. Fixed to `startD || endD || new Date()`. Affects Smartsheet, Jira, and other imports where milestones only have a due date column. 29/29 test files passing. |
| **0.46.4** | 2026-03-28 | B57 fix: PNG/SVG export missing single sub-swimlane. When a swimlane had exactly one sub-swimlane, `buildExportSVG()` rendered it as a simple flat label instead of the split layout (rotated main name + sub-swimlane name). Root cause: `hasSubs` check used `subs.length > 1` (requiring 2+ sub-swimlanes) instead of `subs.length > 0`, which the on-screen `renderTL()` correctly uses. Fixed both label rect pass (line 6255) and label text pass (line 6261). 29/29 test files, 3363+ tests passing. |
| **0.46.0** | 2026-03-13 | Licensing simplification: free-unlock + beta removal. (1) Unlocked all 9 gated features for free tier — feature ranks set to `0` (hooks preserved at all 18 enforcement points for future re-gating by bumping to `1`). All themes now free (`freeThemes` includes warm, cool, light, midnight). PRO badges, menu gating, and export watermark enforcement are no-ops at rank 0. (2) Free tier limits changed to inclusive: 5 swimlanes and 25 items allowed (`<=` instead of `<`), 6th/26th blocked. (3) Removed `beta_boardroom` tier entirely — deleted from `_TIER_CONFIG_DEFAULTS`, removed `?tier=` URL param handling, `?ref=gm` auto-mapping, `tls3_tier`/`tls3_ref` localStorage keys, "Beta access activated!" toast, and all `isBeta` UI checks. Removed `_tier`/`_tierRef` properties. (4) GitHub Pages bypass changed from `beta_boardroom` to `boardroom` tier. (5) `_resolvedTier` fallback simplified: dev override `tls3_devTier` → cached license → `'free'` (no legacy `_tier` fallback). (6) License UI: expired tier now shows correct display name from `lic.tier`; dev override guard prevents `_populateLicenseSection` from overwriting `_resolvedTier`; upgrade summary updated to reflect only swimlane/item limits. (7) `_clearLicenseStorage` simplified (no `tls3_tier` cleanup, no `_tier` reset). (8) Dev Panel: removed "Legacy Tier" and "Tier Ref" state displays, removed `beta_boardroom` from tier simulation dropdown, removed "or beta URL param" help text. (9) New `test_licensing.js` suite: 240 tests across 21 sections covering tier resolution, feature gates, limit gates, license lifecycle, UI state computation, deactivation, GitHub Pages bypass, dev overrides, menu gating, future-proofing hooks, and `deepClone` helper preserving `Infinity` values. 29/29 test files, 3603 tests passing. |
| **0.44.28** | 2026-03-10 | UI polish: toolbar & discoverability improvements. (1) Moved Help from toolbar `?` button to File dropdown as `📖 Help` menu item — reduces toolbar clutter, groups with file-related actions. (2) Settings button upgraded from bare `⚙` emoji to SVG gear icon + "Settings" text label, matching File/Add/View/Tools toolbar conventions. (3) Contact Support form (`📝 Contact Support`) accessible from File dropdown and Settings modal footer — in-app support form with Type (Bug/Feature/Question/Other), Category (11 options including Licensing & billing), Severity, and Description fields. Generates `mailto:` email to `timelinestudiosupport@gmail.com` with structured subject (`[Type] Category`) and body (browser, version, description). Clipboard fallback copies full email content (To, Subject, body) before `mailto:` attempt, with 6-second info toast and static note below submit button. Browser UA parsed to human-readable format (e.g. "Chrome 145 (Windows)"). (4) Safari/Firefox Chrome hint: non-Chromium browsers show "Best experience on Chrome" note at bottom of File dropdown with simplified Chrome logo SVG and "Why?" link that opens Help modal scrolled to new Browser Compatibility section (Chrome/Edge vs Safari/Firefox file handling differences). (5) Settings modal footer gains `?` circle button (opens Help) and `📝 Contact Support` button (opens support form), left-aligned alongside existing Cancel/Apply. (6) Added `_version` property to App object for version tracking in support emails. New CSS: `.dd-chrome-hint` for browser hint, `.s-footer-help` circle button and `.s-footer-support` in settings footer. 28/28 test files passing. |
| **0.44.27** | 2026-03-09 | Overlay positioning fix + always-on label padding. Fixed misalignment of absolute-positioned overlay elements (float labels, TTT labels, weekend shading, holiday shading, today marker, vertical lines) when `_fitLeftPad > 0`. Root cause: overlays positioned relative to `#tl-body`'s padding box, not the content area where `.sw-row` items live. Fix: introduced `#tl-overlays` wrapper div (`position:absolute; left:_fitLeftPad; pointer-events:none`) that centralizes the padding offset for all `dX`-based overlay elements — scalable to any future overlays added inside the wrapper. Also made `_fitLeftPad` computation always-on: moved from `fitToContent()` into `renderTL()` so label-clipping prevention applies at every zoom level and render, not just after fit-to-content. Removed stale `_fitLeftPad = 0` resets from `doZoom()`, `smartZoomForScale()`, timescale handler, and toast switchFn. Toast condition in `fitToContent()` uses local `willClip` variable (computed before `sched()` triggers `renderTL()`). `dep-svg` stays outside wrapper (uses `getBoundingClientRect()` coordinates, already padding-aware). 74 new regression tests in `test_overlay_padding.js` across 5 sections: `_fitLeftPad` computation (12 tests), zoom/render consistency (10 tests), fit-to-content coverage (11 tests), header cell continuity (26 tests), overlay offset consistency (15 tests). 27/27 test files, 538/538 assertions passing. |
| **0.44.26** | 2026-03-09 | Resize visual feedback: tooltip + bottom bar + header highlighting. When dragging task start/end resize handles, now shows the same visual feedback as drag-move: (1) cursor tooltip with day delta via `_fmtDragDelta()` (e.g. "+7d", "-2w 1d"); (2) bottom status strip showing both dates with the dragged side transitioning (left resize: "Start: Mar 9 → Mar 2    End: Mar 22", right: "Start: Mar 9    End: Mar 22 → Mar 29") plus duration change; (3) header column highlighting spanning the task's date range. Duration display is durMode-aware — calendar tasks show "Duration: 14d → 21d", working-day tasks show "Duration: W:10d → 15d  C:14d → 21d" (matching `_fmtDurLabel` pattern). Header highlighting uses `_resizeHdr` flag applied inside `renderTL()` after `innerHTML` rebuild to survive the async rAF render cycle. Escape key cancels resize (reverts dates/duration, pops undo stack, cleans up all feedback). Orphan cleanup on start removes leftover elements from prior aborted resizes. Reuses existing `.drag-delta-tip`, `.drag-date-strip`, `.drag-target` CSS — no style changes. 26/26 test files, 464/464 assertions passing. |
| **0.44.25** | 2026-03-08 | Edge case regression tests. `test_edge_cases.js` (26 tests, 81 assertions) across 6 sections: (1) Pinned items in chains — scheduler and `propagateFrom()` skip pinned items, `calculateFloat()` uses CPM topology not actual position, `getViolatedDepIds()` catches position violations independently; (2) Scheduling pass convergence — verified 1 topo-sorted pass is always sufficient for any DAG (6-item, 10-item chains, diamond, fan-out), 5-pass limit is purely defensive; (3) Zero-duration tasks — `endDate=startDate`, `_depEnd()=startDate+1` (same as milestone), documented float=1 bug caused by JS falsy `0` in forward pass (`0||(days(s,e)+1)=1`) while backward pass uses `0||0=0`; (4) Float >= 0 invariant with violation independence — float always non-negative without external deadlines, `getViolatedDepIds()` checks actual positions independently from CPM float; (5) FF/SF float calculation — FF aligns endings (both float=0), SF is intentionally the loosest dep type (successor gets large positive float), FS dominates in mixed branching; (6) Load-state consistency — `_float` deleted on save and correctly restored, stale endDates freshened by both `calculateFloat()` and `runSchedule()`. Combined with `test_stale_data.js` (31 tests, 95 assertions from v0.44.24). 27/27 test files, 464/464 assertions passing. |
| **0.44.24** | 2026-03-08 | Critical Path pill + stale endDate fix. Added header bar pill indicator for Critical Path mode (orange fire icon, click-to-dismiss). Fixed stale endDate bug where root items or durMode changes caused incorrect float calculations — endDates are now defensively freshened in `calculateFloat()`, `runSchedule()`, and `_recalcNonWorkingDays()`. `test_stale_data.js` regression tests (31 tests, 95 assertions) covering stale root endDates, settings changes mid-lifecycle, durMode changes, scheduling mode transitions, dependency changes in auto mode, holiday edge cases, propagation chains, and critical path through settings changes. Verified 16 assertions fail against pre-fix engine. 26/26 test files, 464/464 assertions passing. |
| **0.44.23** | 2026-03-08 | SF dependency support + bug hardening pass. Added Start-Finish (SF) link type across all engine paths: `calcEarlyStart()` forward scheduling (app.js:1259), `calculateFloat()` backward pass with SF→LS constraint separate from FS/FF→LF path (app.js:1290–1348, mirrored in mock-engine.js:257), `getViolatedDepIds()` violation detection (app.js:1384), and UI dependency chip dropdown with SF option + updated help text (app.js:4432). Preserved B38 asymmetric FS/FF backward-pass float fix from v0.44.21. Import cycle guard: `wouldCreateCycle()` BFS check during advanced import prevents circular dependency edges, with `skippedCircular` counter in result message (app.js:3890, 3915). Test runner EPERM fallback: `run-all.js` detects blocked `child_process.spawn` and falls back to in-process `require()` execution with module cache isolation. Docs updated: README, CLAUDE.md, dependency-prd.md all reflect SF as supported type. Also includes beta tier URL param activation (`?tier=` / `?ref=gm` → localStorage persistence with URL cleanup and toast). 25/25 test files, 464/464 assertions passing. |
| **0.44.22** | 2026-03-07 | App icon update: replaced inline SVG logo with actual PNG asset from landing page (base64-embedded, canvas-resized to 48x48 with square crop to fix aspect-ratio artifact in original 220x212 source). Theme-adaptive via CSS `filter` per theme: `hue-rotate(152deg) saturate(0.65)` for Warm, `brightness(0.92)` for Light, `hue-rotate(11deg) brightness(1.25)` for Midnight, unfiltered for Cool. Favicon updated from SVG to canvas-rendered PNG using `ctx.filter` in `applyTheme()` for matching theme colors. |
| **0.44.21** | 2026-03-06 | Float calculation fix: FS/FF backward pass in `calculateFloat()` used indirect calendar-day approximation (`cand = pEF + (sls - fwdContrib)`) that produced incorrect LF values when working-day lag crossed weekends differently than the reference offset. Could inflate or deflate float by 1-2+ days. Replaced both FS and FF formulas with direct `_addLagWorkingDays(sls, -lag, sdm)` pattern, matching the already-correct SS backward pass. FF simplified to use pre-computed `lf.get(s.id)` instead of recomputing successor LF from LS + duration. |
| **0.44.20** | 2026-03-04 | F43: Import source tool presets + scheduling options. Source tool auto-detection (`detectSourceTool()`) with `_IMP_PRESETS` registry (Smartsheet, Jira, Asana, Monday.com, MS Project, Generic). Source dropdown UI with confidence badges. Preset-aware column mapping via `_mergedAliases()`. `parseJSON()` for JSON array import. `parseDuration()` multi-format parser. Smartsheet predecessor off-by-one correction. Import Options panel: duration mode (Cal/Work), default duration, date format (Auto/US/EU/ISO), snap-to-working-days. `_impParseDate()` for EU date format. Working-day end-date calculation in `doAdvancedImport()`. 37 import sample files + 249-test suite. |
| **0.44.19** | 2026-03-04 | Ctrl+Shift+Scroll fine zoom fix: Chrome converts deltaY to deltaX when Shift held, causing both directions to zoom out. Now falls back to deltaX when deltaY is zero. |
| **0.44.18** | 2026-03-04 | Presenter mode: footer text overlap fix + fullscreen Esc bug. Footer overlap: replaced `padding-bottom:14px` on toolbar with `margin-bottom:12px` on `.pres-tools` (only when labels visible via `:not(.pres-has-opts)`), pushing hint text below `::after` label zone. Fullscreen Esc: Chrome dispatches keydown to capture-phase handler during fullscreen exit, creating unwanted exit popup — fixed with `document.fullscreenElement` guard. |
| **0.44.17** | 2026-03-04 | Presenter mode polish: colored cursor trail, key label hiding, footer hint spacing. Cursor trail: replaced static `::before` color bar on buttons with canvas-rendered cursor trail reusing laser trail mechanics (per-tool colors/sizes, hex→RGB parsing for radial gradients). Key labels: added `pres-has-opts` class to hide `::after` labels when opts row (color picker + size slider) visible. Footer hint: adjusted spacing with `margin-top:2px` and `padding:3px 6px 1px`. Removed all `pres-clr` CSS class and `--tool-clr` CSS variable logic. |
| **0.44.16** | 2026-03-04 | Presenter mode: 4 follow-up fixes from user testing. Labels: replaced `pres-labels-row` with `::after` pseudo-elements via `data-key` attributes for pixel-perfect centering over each button. Dropdown access: bumped `#toolbar` z-index 100→550 (canvas z:500 was blocking dropdowns trapped in toolbar's stacking context). Color indicator: `::before` pseudo-element with `--tool-clr` CSS variable shows active tool color. Fullscreen: `F` key handler in `_presKeyHandler()` + updated hint text. |
| **0.44.15** | 2026-03-04 | Presenter mode: 13 fixes & enhancements. Z-index hierarchy (toolbar z-600, exit popup z-601). Enriched tooltips for pen/highlight (auto-fade info). Scroll wheel/trackpad passthrough via wheel event forwarder. Hotkey toggle (pressing active tool's key deactivates it). Separate pen/highlight color defaults (highlighter=yellow). Middle mouse pan (`e.button===1`). Draggable toolbar with grip handle (`⠿`). X closes directly / Esc shows prompt / double-Esc confirms. Compact toolbar (smaller padding/icons). Key labels as separate row below icons. Yellow pulse glow animation on open. `togglePresent` shortcut in `SHORTCUT_ACTIONS`. Viewport-aware exit prompt positioning. |
| **0.44.14** | 2026-03-04 | F63: Presentation Mode — Laser Pointer, Drawing Pen & Highlighter. Full-viewport canvas overlay (`#pres-canvas`) with `pointer-events` toggling. Five tools: Laser (red dot + fading trail), Pen (freehand strokes, 3s auto-fade), Highlighter (wide translucent strokes, 5s auto-fade), Pan (hand cursor, scroll passthrough), Clear All. Toolbar with emoji icons, opts row (color picker + size slider), keyboard activation (L/D/H/P/C), Esc to exit. Content-relative stroke storage (survives scroll). Canvas sized via `ResizeObserver`. `togglePresent` shortcut action added to `SHORTCUT_ACTIONS` + `_scDispatch`. |
| **0.44.13** | 2026-03-04 | F50: Tools dropdown Screenshot & Export collapsible section. Reorganized flat screenshot items into a collapsible "📷 Screenshot & Export" section with toggle arrow (mirrors MRU pattern — defaults collapsed, resets on dropdown close). Section groups Screenshot Viewport, Screenshot Full, and four new export items (SVG Full, PNG HD, CSV, JSON) — previously only accessible via Settings → Export. Rich theme-aware tooltips on all export items. Inline tags: SVG "(Full)", PNG "(HD)". Footer hint: "Exports capture full timeline". New IDs use `btn-tools-exp-*` prefix (avoids collision with Settings `btn-exp-*`). Uses 📋 for JSON export instead of 📄 (macOS monochrome fix). Settings Export section retained as alternate entry point. |
| **0.44.12** | 2026-03-04 | B56 fix: File picker greys out target `.tlproj` on reconnect. When `_tryReconnect()` failed (user dismissed permission prompt or handle expired), falling back to `openFile()` with a stale IDB handle could cause Chrome to grey out the specific file in the picker. Fix: `_clearHandle()` before `openFile()` fallback in subtitle click handler. Same cleanup added to `saveFile()` auto-reconnect path (Ctrl+S with stale handle). Also broadened file picker MIME type to include `application/octet-stream` alongside `application/json` for `.tlproj` files — ensures macOS UTI matching doesn't reject files with unregistered custom extensions. |
| **0.44.11** | 2026-03-04 | B55 fix: File dropdown emoji monochrome on macOS. 📄📂💾 rendered monochrome on both Chrome and Safari on Mac — a macOS Apple Color Emoji font quirk with specific codepoints (CSS color removal, font-family, and VS16 variation selector all failed to fix). Swapped to color-rendering alternatives: 📄→✨ (Sparkles/New), 📂→🗂️ (Card Index/Open), 💾→🗃️ (Card File Box/Save). Also removed `color:var(--acc)` from broad `.save-dd-item span` rule, targeted to `.dd-shortcut` only. Safari/Firefox ⬇ Download icon (U+2B07) unchanged. |
| **0.44.10** | 2026-03-04 | B54 follow-up: Ctrl+digit display fix for Safari/Firefox Mac. `_displayCombo()` now shows ⌃ (physical Ctrl) instead of ⌘ for digit combos on non-FS browsers (Safari/Firefox) because Cmd+0–9 is reserved by all Mac browsers for tab switching. Chrome Mac shows ⌘ for all combos since capture-phase JS can block browser shortcuts. Lookahead regex `Ctrl\+(?=(?:Shift\+\|Alt\+)*\d)` handles bare digits and Shift/Alt+digit combos. Uses `_hasFS` as Chromium proxy (no UA sniffing). ⌃ added to capitalize regex `[⌘⇧⌥⌃]`. Cross-platform keyboard behavior matrix added to `docs/CLAUDE.md`. 16 new display combo tests in `test_shortcuts.js` (165 total). |
| **0.44.9** | 2026-03-04 | B54 fix: Safari shortcut recorder Cmd key. The capture-phase keydown handler in `_startScRecord()` called `e.preventDefault()` + `e.stopPropagation()` unconditionally on every keydown, including modifier-only presses (Cmd/Ctrl/Shift/Alt alone). Safari treats a prevented Meta keydown as "handled" and suppresses subsequent Cmd+letter keydown events — so the recorder never saw the actual combo. Fix: moved `preventDefault()`/`stopPropagation()` to after the `_normalizeKey()` check, so modifier-only keydowns pass through untouched. Chrome behavior unchanged (Chrome doesn't suppress Cmd+key after prevented Meta keydown). 20 new recorder handler tests added to `test_shortcuts.js` (149 total). |
| **0.44.8** | 2026-03-04 | Safari/Firefox file handling UI adaptation + Today Marker toggle fix. Added `_hasFS` feature flag (`!!window.showSaveFilePicker`) for browser capability detection at parse time. On non-FS browsers (Safari, Firefox): File dropdown shows New, Open (direct picker), Download (Ctrl+S), Share via Link — hides Save As and Recent Files section. File subtitle shows filename without "click to reconnect", shows "Ctrl+S to download" for new projects. Startup reconnect, MRU cache warming, and handle validation all guarded by `_hasFS`. Chrome/Edge behavior completely unchanged. Bug fix: Today Marker toggle in Project Settings collapsed to ~2px wide due to CSS specificity conflict (`.form-group>label{display:block}` overriding `.toggle-switch`); fixed with `.form-group>.toggle-switch{display:inline-flex}`. Toggle gap changed from `justify-content:space-between` to `gap:10px`. |
| **0.44.7** | 2026-03-03 | F60: Dependency display filtering. New `depFilter` project property ('all'\|'swimlane') with sub-options under Show Dependencies in Settings (Appearance → Visibility). "Within swimlane only" hides cross-swimlane dependency arrows while preserving scheduling engine behavior — addresses beta user request for clean exec-level reviews. View dropdown gains Dependencies section (Off/All/Within Swimlane). New `cycleDeps` keyboard shortcut action (View category, no default binding). Filter applied in both on-screen `rDeps()` and export SVG rendering. Settings UI converted Show Dependencies from toggle switch to `.shading-block` pattern with expandable radio sub-options. 3,053 tests pass across 24 files. |
| **0.44.6** | 2026-03-02 | B53 fix: Header/body scroll alignment. Three root causes fixed: (1) fractional `cw` caused subpixel rounding divergence between header (`overflow:hidden`) and body (`overflow:auto`) scroll containers — `met()` now rounds `cw` to integer, `dX()`/`dXMid()` return `Math.round()` values, weekend shading uses integer tiling formula; (2) body included panel padding in its width but header didn't, causing different `scrollWidth` — header now includes panel padding; (3) body's vertical scrollbar (~15px) gave it a larger max `scrollLeft` than the header — header width now adds dynamically measured scrollbar width (`offsetWidth - clientWidth`) so both containers have matching scroll ranges at all viewport sizes. Header corner `box-sizing:border-box` fix ensures label columns match exactly. Header cells use `position:absolute` (was `flex`). 464 tests pass. |
...
| F66 | **Embedded checkout & go-live** | Embed Lemon Squeezy overlay checkout directly in the License & Upgrades section so users can purchase a Boardroom license without leaving the app. Auto-activate license on successful payment. End-to-end test matrix across browsers. **See:** `docs/PHASE-D-GO-LIVE.md` for full plan. **Depends on:** Phase C (done — visual polish, PRO badges, upgrade CTAs). | M | :orange_circle: P1 | :white_circle: Open |
...
| F61 | **Custom import config save** | Allow users to save and recall named import configurations (column mappings, scheduling options, source tool presets). Use 🏷️ (Label/Tag) emoji as the visual icon for saved configs. **Related to:** F43 (import scheduling options), F34 (import from context menu). | S | :large_blue_circle: P3 | :white_circle: Open |
| F62 | **Onboarding help integration** | Cohesive help/onboarding system for new users incorporating 📎 (Paperclip) emoji. Could include contextual tips, first-use walkthroughs, or an interactive guide that bridges the gap between the Help (?) button and the onboarding flow. **Related to:** F48 (onboarding highlights), F13 (shortcut discoverability). | M | :large_blue_circle: P3 | :white_circle: Open |
| F64 | **Tool-specific import parsers & quirks** | **[Plan — see `plans/drifting-sparking-crane.md` for full roadmap]** (A) MS Project XML parser: `DOMParser`-based conversion of MSPDI XML → `{headers, rows}` for mapping pipeline. OutlineLevel hierarchy → swimlanes, ISO 8601 durations, PredecessorLink dep types (0=FF,1=FS,2=SF,3=SS), lag in tenths-of-minutes, milestones, PercentComplete. Test against 5 sample XML files. (B) Jira Sprint Decoder: custom UI for defining `Sprint Name → Date Range` since Jira CSV exports include sprint names but NOT sprint dates. Pre-populate unique sprint names, user fills dates (with optional cadence auto-fill helper). Saved/reusable decoder config. Epic Link → Swimlane alias. (C) Jira issue-link dependencies: resolve deps by issue key instead of row number (`"Outward issue link (Blocks)"` columns). (D) Monday.com Timeline column: split `"2026-01-15 - 2026-01-30"` compound column into Start + End. (E) Nested JSON: `{lanes:[{name, items:[...]}]}` tree-to-items conversion. **Depends on:** F43 (done), Step 1 presets (done). | L | :orange_circle: P1 | :memo: Plan |
| F65 | **Import summary card** | Post-import report showing what was created and any warnings. Metrics: items created, swimlanes created/matched, statuses created/matched, deps resolved, rows skipped. Warnings: unmatched predecessors, unparseable dates, duplicate names, items with default dates. Clickable: jump to first imported item on timeline. **Depends on:** F43 (done). | M | :yellow_circle: P2 | :white_circle: Open |
...
- **F66** — Embedded checkout & go-live (M) — embed Lemon Squeezy overlay checkout in Settings; see `docs/PHASE-D-GO-LIVE.md`
- **F64** — Tool-specific import parsers & quirks (L) — :memo: Plan (MS Project XML, Jira sprint decoder + issue-link deps, Monday Timeline, nested JSON)
...
- **F65** — Import summary card (M) — post-import report with metrics & warnings
