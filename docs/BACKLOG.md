...
| B52 | **Shift+drag off-by-one day** | Reported by beta user (Sean): Shift+drag (vertical lock) may shift the item date by 1 day. **Note:** B28 (v0.29.2) fixed a related `parseInt→parseFloat` issue in shift+drag — this may be a regression or a different edge case. Needs repro and verification. **Source:** Beta user feedback (Feb 2026). | XS | :yellow_circle: P2 | :white_circle: Open |
...
- **B52** — Shift+drag off-by-one day (XS) — beta user report, needs verification
