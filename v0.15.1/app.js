...
const App={
  ...
  init(){
    ...
    ['toolbar','main-content','props-panel','panel-body','panel-title',
     ...
     'tl-body-wrap','tl-sl-labels','tl-body-scroll','tl-body','tl-watermark',
  ...
  renderTL(){
    ...
    for(const sl of p.swimlanes){
      ...
      if(!isCollapsed&&hasSubs){const mainW=Math.min(60,160/2);labelsH+=`<div class="sl-lbl-main" style="width:${mainW}px;min-width:${mainW}px;writing-mode:vertical-rl;text-orientation:mixed;transform:rotate(180deg)">${U.esc(sl.name)}</div><div class="sl-lbl-subs">`;for(const{ssId,h}of subMeta){const ss=sl.subSwimlanes.find(s=>s.id===ssId);labelsH+=`<div class="sl-sub-lbl" style="height:${h}px">${ss?U.esc(ss.name):''}</div>`}labelsH+=`</div>`}
  ...
  showHelp(){
    ...
    <tr><td style="padding:3px 8px"><kbd style="background:var(--bg2);padding:1px 5px;border-radius:3px;font-family:var(--mono)">Ctrl+Shift+P</kbd></td><td style="padding:3px">Propagate to Successors</td><td></td><td></td></tr>
    </tbody></table>
