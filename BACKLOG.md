...
| F29 | **Pan mode** | **[Done — v0.28.0]** Middle-mouse drag for instant pan + toggle-based Pan Mode (Tools → ✋) with bindable keyboard shortcut. 2D scrolling (horizontal + vertical) with grab/grabbing cursor feedback. Pan and Lasso modes are mutually exclusive. Escape exits Pan Mode. Originally spec'd as right-click drag but pivoted to middle-mouse + toggle to avoid context menu conflicts and browser Space-scroll interference. Capture-phase `preventDefault` on scroll container prevents native middle-click auto-scroll. | M | :orange_circle: P1 | Done |
