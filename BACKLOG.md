...
| # | Title | Description | Size | Priority | Status |
|---|-------|-------------|------|----------|--------|
| F28 | **Bulk drag-and-drop to new swimlane** | Multi-select group move: when dragging multiple selected items to a new swimlane, maintain relative positioning between items on drop. Preview target swimlane expanding to accommodate incoming items. Two-tier ghost preview — darker ghost for primary dragged item, lighter ghosts for other selected items showing snap positions. Follow-up to v0.27.0 drag overhaul. | L | :orange_circle: P1 | :white_circle: Open |
| F31 | **Item-anchored zoom** | When items are selected and you zoom on the timeline, zoom "around" the selection keeping it fixed at the visual center — similar to how design tools anchor zoom to your focus point. For multi-select, use the centroid of the first and last selected items (by date) as the anchor point. If no items are selected, fall back to current behavior (zoom toward center of viewport). | M | :orange_circle: P1 | :white_circle: Open |
| F32 | **Fit to selected items** | Fit the viewport to show only the currently selected items (instead of all items). Leverage existing `fitToContent()` logic and iterative zoom solver — same treatment for label positions, status inline display, edge labels, text measurement via `_mt()`, collapsed swimlane exclusion, etc. Available as a toolbar button and keyboard shortcut. If nothing is selected, fall back to regular fit-to-content. | M | :orange_circle: P1 | :white_circle: Open |
| F33 | **Auto-fit swimlane heights** | Add an "Auto Fit Heights" button underneath the Expand All / Collapse All swimlane buttons in the View dropdown. Automatically shrinks swimlane and sub-swimlane heights to tightly fit their content (items + padding) without excess whitespace. Also available per-swimlane and per-sub-swimlane in the swimlane editor — show an "Auto Fit" button when the lane or any of its subs could be narrowed. **Must NOT remove the ability to expand swimlanes/subs larger than needed** — users rely on extra space for adding items or drag/drop targets. Auto-fit reduces height but never prevents manual resize back up. Should be configurable as a keyboard shortcut via `SHORTCUT_ACTIONS`. | M | :orange_circle: P1 | :white_circle: Open |
...
- **F33** — Auto-fit swimlane heights (M)
