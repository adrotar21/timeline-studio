...
|---------|------|---------|
| **0.35.0** | 2026-02-18 | UI polish pass: Theme-aware tooltips (CSS vars instead of hardcoded colors), input focus glow (`box-shadow:0 0 0 3px var(--accL)` on all 9 focus rules), custom scrollbars (webkit + Firefox, theme-matched), button hierarchy (`.btn-primary` font-weight + shadow), alternating data table row shading (`.015` opacity, non-conflicting with hover/selection), toast exit animation (fade+slide out via `.toast-out` class), context menu keyboard shortcut hints (Del, Ctrl+Shift+P), modal entrance/exit animations (opacity+scale via `.visible` class with `showModal()`/`hideModal()` helpers, 20+ call sites converted), dropdown entrance animations (opacity+transform via `.show` class replacing `.hidden` toggling). |
