...
|---------|------|---------|
| **0.18.2** | 2026-02-11 | Expand All now expands sub-swimlanes too (button state reflects sub collapse). Vertical main swimlane text auto-scales font-size down (min 8px) when subs are collapsed and height is short, with CSS `overflow:hidden` safety net. Export SVG mirrors dynamic font-size. Minimized sub height bumped 16px→20px with slightly larger button (13×13) and padding. |
| **0.18.1** | 2026-02-11 | Patch: Fixed parent swimlane resize handle cumulative delta bug when subs exist (captured startLastH once at mousedown). Sub-swimlane dividers now 1px thin lines with transparent 6px resize hit targets. Auto-minimize parent when all subs minimized; auto-expand parent when expanding a sub. Minimized sub-swimlanes slimmed from 28px to 16px with tighter button/label sizing. |
